...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome = (f"<b>🚀 NOC SYSTEM ONLINE v3.0</b>\n"
               ...
               f"Node: 🖥️ <b>RB450Gx4</b> | Jalur: <b>Biznet</b>\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pool = connect_mt()
    if not pool:
        await update.message.reply_text("❌ <b>MikroTik kaga respon!</b> Cek IP/Firewall.", parse_mode='HTML')
        return
    api = pool.get_api()
...
async def do_search_ip(u, c):
    ...
    await u.message.reply_text(msg if res else "Kaga nemu!", parse_mode='HTML'); pool.disconnect(); return ConversationHandler.END
...
async def do_ping_test(u, c):
    ...
    except: await u.message.reply_text("❌ IP kaga valid!")
